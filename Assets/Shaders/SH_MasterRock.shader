...
Shader "SH_MasterRock"
{
	...
	LOD 0
		...
		Pass
		{
			...
			uniform float _SunColorInfluence1;
			uniform float4 _RockColor0;
				...
				) : SV_Target
			{
				...
				float3 worldToObj43 = mul( unity_WorldToObject, float4( worldPos, 1 ) ).xyz;
				#ifdef _OBJECTSPACE_ON
				float3 staticSwitch42 = worldToObj43;
				#else
				float3 staticSwitch42 = worldPos;
				#endif
				float lerpResult21 = lerp( tex2D( _Rock, ( (staticSwitch42).xy / _TextureWorldScale ) ).r , tex2D( _Rock, ( (staticSwitch42).zy / _TextureWorldScale ) ).r , WorldNormal.x);
				float3 lerpResult51 = lerp( (_RockColor0).rgb , (_RockColor1).rgb , lerpResult21);
				float4 transform101 = mul(unity_ObjectToWorld,float4( 0,0,0,1 ));
				float gradientNoise7 = GradientNoise(( (transform101).xz / 100.0 ),100.0);
				#ifdef _COLORVARIATION_ON
				float3 staticSwitch103 = ( lerpResult51 + ( gradientNoise7 * _VariationIntensity ) );
				#else
				float3 staticSwitch103 = lerpResult51;
				#endif
				float3 worldToObj166 = mul( unity_WorldToObject, float4( worldPos, 1 ) ).xyz;
				float4 triplanar45 = TriplanarSampling45( _Grunge, ( (worldToObj166).xyz / _GrungeScale ), WorldNormal, 1.0, float2( 1,1 ), 1.0, 0 );
				#ifdef _USEUV_ON
				float staticSwitch158 = tex2D( _Grunge, ( IN.ase_texcoord9.xy / _GrungeScale ) ).r;
				#else
				float staticSwitch158 = triplanar45.x;
				#endif
				float temp_output_31_0 = ( 1.0 - IN.ase_color.g );
				float temp_output_50_0 = ( 1.0 - ( staticSwitch158 * temp_output_31_0 ) );
				float saferPower54 = abs( ( 1.0 - IN.ase_color.b ) );
				float3 temp_output_52_0 = ( staticSwitch103 + ( staticSwitch158 * pow( saferPower54 , 2.0 ) * _HighlightIntensity ) );
				#if defined(_VERTEXCOLORMASKS_NONE)
				float3 staticSwitch65 = staticSwitch103;
				#elif defined(_VERTEXCOLORMASKS_DIRT)
				float3 staticSwitch65 = ( staticSwitch103 * temp_output_50_0 );
				#elif defined(_VERTEXCOLORMASKS_EDGEHIGHLIGHT)
				float3 staticSwitch65 = temp_output_52_0;
				#elif defined(_VERTEXCOLORMASKS_BOTH)
				float3 staticSwitch65 = ( ( staticSwitch103 + temp_output_52_0 ) * temp_output_50_0 );
				#else
				float3 staticSwitch65 = ( staticSwitch103 * temp_output_50_0 );
				#endif
				float saferPower79 = abs( WorldNormal.y );
				float temp_output_38_0 = saturate( ( ( 1.0 - pow( saferPower79 , 2.0 ) ) * ( worldPos.y / _SandMaskHeight ) ) );
				#ifdef _SANDMASKON_ON
				float3 staticSwitch73 = ( staticSwitch65 * temp_output_38_0 );
				#else
				float3 staticSwitch73 = staticSwitch65;
				#endif
				
				float3 desaturateInitialColor7_g2 = SunColor;
				float desaturateDot7_g2 = dot( desaturateInitialColor7_g2, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar7_g2 = lerp( desaturateInitialColor7_g2, desaturateDot7_g2.xxx, 1.0 );
				float3 normalizeResult127 = normalize( SunDirection );
				float fresnelNdotV128 = dot( WorldNormal, normalizeResult127 );
				float fresnelNode128 = ( _SandShadeBias + _SandShadeScale * pow( 1.0 - fresnelNdotV128, _SandShadePower ) );
				float3 lerpResult138 = lerp( (_Sand).rgb , (_SandShade).rgb , saturate( fresnelNode128 ));
				#ifdef _SANDMASKON_ON
				float3 staticSwitch41 = ( ( desaturateVar7_g2 * _SunColorInfluence1 ) * ( 1.0 - temp_output_38_0 ) * lerpResult138 );
				#else
				float3 staticSwitch41 = float3( 0,0,0 );
				#endif
		...
		Pass
		{
			...
			)
			{
				...
				float4 triplanar45 = TriplanarSampling45( _Grunge, ( (worldToObj166).xyz / _GrungeScale ), WorldNormal, 1.0, float2( 1,1 ), 1.0, 0 );
				#ifdef _USEUV_ON
				float staticSwitch158 = tex2D( _Grunge, ( IN.ase_texcoord8.xy / _GrungeScale ) ).r;
				#else
				float staticSwitch158 = triplanar45.x;
				#endif
				float temp_output_31_0 = ( 1.0 - IN.ase_color.g );
		...
		Pass
		{
				...
				) : SV_Target
			{
				...
				float lerpResult21 = lerp( tex2D( _Rock, ( (staticSwitch42).xy / _TextureWorldScale ) ).r , tex2D( _Rock, ( (staticSwitch42).zy / _TextureWorldScale ) ).r , ase_worldNormal.x);
				float3 lerpResult51 = lerp( (_RockColor0).rgb , (_RockColor1).rgb , lerpResult21);
				float4 transform101 = mul(unity_ObjectToWorld,float4( 0,0,0,1 ));
				float gradientNoise7 = GradientNoise(( (transform101).xz / 100.0 ),100.0);
				#ifdef _COLORVARIATION_ON
				...
				float3 staticSwitch103 = lerpResult51;
				#endif
				float3 worldToObj166 = mul( unity_WorldToObject, float4( ase_worldPos, 1 ) ).xyz;
				float4 triplanar45 = TriplanarSampling45( _Grunge, ( (worldToObj166).xyz / _GrungeScale ), ase_worldNormal, 1.0, float2( 1,1 ), 1.0, 0 );
				#ifdef _USEUV_ON
				float staticSwitch158 = tex2D( _Grunge, ( IN.ase_texcoord5.xy / _GrungeScale ) ).r;
				#else
				float staticSwitch158 = triplanar45.x;
				#endif
				float temp_output_31_0 = ( 1.0 - IN.ase_color.g );
				...
				float temp_output_38_0 = saturate( ( ( 1.0 - pow( saferPower79 , 2.0 ) ) * ( ase_worldPos.y / _SandMaskHeight ) ) );
				#ifdef _SANDMASKON_ON
				float3 staticSwitch73 = ( staticSwitch65 * temp_output_38_0 );
				#else
				float3 staticSwitch73 = staticSwitch65;
				...
				float3 desaturateVar7_g2 = lerp( desaturateInitialColor7_g2, desaturateDot7_g2.xxx, 1.0 );
				float3 normalizeResult127 = normalize( SunDirection );
				float fresnelNdotV128 = dot( ase_worldNormal, normalizeResult127 );
				float fresnelNode128 = ( _SandShadeBias + _SandShadeScale * pow( 1.0 - fresnelNdotV128, _SandShadePower ) );
				float3 lerpResult138 = lerp( (_Sand).rgb , (_SandShade).rgb , saturate( fresnelNode128 ));
				#ifdef _SANDMASKON_ON
				float3 staticSwitch41 = ( ( desaturateVar7_g2 * _SunColorInfluence1 ) * ( 1.0 - temp_output_38_0 ) * lerpResult138 );
...
Node;AmplifyShaderEditor.Vector2Node;24;-1356.09,-1248.422;Inherit;False;Property;_TextureWorldScale;TextureWorldScale;5;0;Create;True;0;0;0;False;0;False;1,1;11.5,6.3;0;3;FLOAT2;0;FLOAT;1;FLOAT;2
...
Node;AmplifyShaderEditor.ColorNode;9;-341.5219,-919.3012;Inherit;False;Property;_RockColor0;RockColor0;3;0;Create;True;0;0;0;False;0;False;0,0,0,0;0.4056604,0.2405911,0.1128961,1;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ColorNode;10;-328.8841,-758.818;Inherit;False;Property;_RockColor1;RockColor1;4;0;Create;True;0;0;0;False;0;False;0,0,0,0;0.4528302,0.3182628,0.1388395,1;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;130;1564.261,-1420.726;Inherit;False;Property;_SandShadeBias;SandShadeBias;15;0;Create;True;0;0;0;False;0;False;0;-0.78;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;131;1558.152,-1337.239;Inherit;False;Property;_SandShadeScale;SandShadeScale;14;0;Create;True;0;0;0;False;0;False;0;2.89;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;132;1643.813,-1264.98;Inherit;False;Property;_SandShadePower;SandShadePower;13;0;Create;True;0;0;0;False;0;False;0;-1.56;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;133;1064.565,-1071.163;Inherit;False;Property;_SandShade;SandShade;9;0;Create;True;0;0;0;False;0;False;0,0,0,0;0.3853091,0.4610046,0.6415094,0;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ColorNode;134;1063.122,-1233.673;Inherit;False;Property;_Sand;Sand;8;0;Create;True;0;0;0;False;0;False;0,0,0,0;0.6415094,0.5497211,0.4579328,0;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.Vector3Node;137;1611.158,-1573.269;Inherit;False;Global;SunDirection;SunDirection;28;0;Create;True;0;0;0;True;0;False;0,0,0;27.4,171.18,354.598;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
Node;AmplifyShaderEditor.StaticSwitch;65;2182.748,-494.7237;Inherit;False;Property;_VertexColorMasks;VertexColorMasks;18;0;Create;True;0;0;0;False;0;False;0;1;1;True;;KeywordEnum;4;None;Dirt;EdgeHighlight;Both;Create;True;True;All;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RangedFloatNode;37;1290.635,604.1499;Inherit;False;Property;_SandMaskHeight;SandMaskHeight;6;1;[Header];Create;True;1;Sand;0;0;True;0;False;1;9.93;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;66;3577.309,-289.3418;Inherit;False;Property;_Smoothness;Smoothness;0;1;[Header];Create;True;1;Global;0;0;False;0;False;0;0.099;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;26;-1017.845,-1423.505;Inherit;True;Property;_Rock;Rock;1;1;[Header];Create;True;1;Rock;0;0;False;0;False;e955a59092a218e4e9861f0d837f8f8f;13d35dfe001ee9d4cbfd86cf83bd5c28;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
WireConnection;169;2;41;0
WireConnection;169;5;66;0
ASEEND*/
//CHKSM=68BDFC6FCF8E5B7219AC4E421911820FE424510B