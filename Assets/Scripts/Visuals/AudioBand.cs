...
namespace Assets.Scripts
{
    ...
    public class AudioBand
    {
        ...
        public void Reset()
        {
            // Reinitialize the relevant fields to their default states.
            _originalSamplesLeft = new float[SAMPLES_COUNT];
            _bandFrequency = new float[BAND_COUNT];
            _bandFrequencyNormilized = new float[BAND_COUNT];
            _bandFrequencyBuffer = new float[BAND_COUNT];
            ...
            // If there are any additional fields or properties to reset, do so here.
            _amplitudeHighest = 5;
            _amplitudeBuffer = 0;
            _amplitude = 0;

            // If _bandFrequencyHighest should be reset to the value of _audioProfile for all bands.
            AudioProfile();
        }
